...
#kiểm tra ng dùng là admin, bác sĩ hay bệnh nhân
def is_admin(user):
...
#kiểm tra thông tin đăng nhập là của admin, bác sĩ hay bệnh nhân
...
#--------------tải và in hóa đơn của bệnh nhân
import io
...
@login_required(login_url='doctorlogin')
@user_passes_test(is_doctor)
def doctor_medicine(request):
    medicines = models.Medicine.objects.all()
    doctor=models.Doctor.objects.get(user_id=request.user.id)
    return render(request, 'hospital/doctor_medicine.html', {'medicines': medicines})
